...
class KolmogorovInterpolantMulticlassClassificationModel:
    ...
    def fit(self, probs: np.ndarray, embeddings: np.ndarray, targets: np.ndarray):
        ...
        self._train_mv_ecdf = self._get_mv_ecdf(embeddings)
    ...
    def predict_proba(self, probs: np.ndarray, embeddings: np.ndarray) -> np.ndarray:
        ...
        mv_ecdf = self._get_mv_ecdf(embeddings)
        d = np.abs(
            self._eval_mv_ecdf(mv_ecdf, embeddings)
            - self._eval_mv_ecdf(self._train_mv_ecdf, embeddings)
        )

        w = np.mean(kolmogorov(np.sqrt(len(embeddings)) * d), axis=1, keepdims=True)
    ...
    @staticmethod
    def _get_mv_ecdf(embeddings) -> List[Callable]:
        return [
    ...
    @staticmethod
    def _eval_mv_ecdf(mv_ecdf: List[Callable], embeddings) -> float:
